...
size_t countLiveNeighbours(size_t row, size_t col)
{
	...
	// your code goes here
		for (int i=-1;i<2;i++)
		{
			for(int j=-1;j<2;j++)
			{
				// don't be afraid to use if statements or switch/case
				R = (size_t) (row + i + config_NE) % config_NE;
				C = (size_t) (col + j + config_ME) % config_ME;
				cell_count = cell_count + (size_t)env[R][C];
			}
		}

		cell_count = cell_count -  (env[row][col]==live ? 1 : 0); // works only because live = 1, dead = 0;
...
void updateCell(size_t r, size_t c)
{
	...
	else if(state_cell==1 &&( live_neighbours<2 || live_neighbours>3))
	{
		update_env[r][c] = dead;
	}
	else
